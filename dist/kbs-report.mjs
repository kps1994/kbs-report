...
var FileManager = class {
  ...
  ensureReportDirectory() {
    const kbsDataFolder = path.join(this.folderPath, "kbs-data");
    if (!fs.existsSync(this.folderPath)) {
      fs.mkdirSync(this.folderPath, { recursive: true });
    } else {
      if (fs.existsSync(kbsDataFolder)) {
        fs.rmSync(kbsDataFolder, { recursive: true, force: true });
      }
    }
  }
...
var KbsReport = class {
  ...
  async onBegin(config, _suite) {
    ...
    await this.dbManager.initialize(
      path6.join(this.folderPath, "kbs-data-history.sqlite")
    );
  }
